...
long test_1 (long p, long k) {
    ...
    if (t_1 == 0) {
        ...
        s_1 = sum_1(p, k);
        if (s_1 == 0) {
            // Found an irregular pair
            return 1;
        }
        // Not an irregular pair
        return 0;
    }
}

long test_2 (long p, long k) {
    long t_2, s_2;
...
long test_4 (long p, long k) {
    long t_4, s_4;
    if (2*k % (p-1) == 2) {
        // test 4 is inconclusive
        return -1;
    }
